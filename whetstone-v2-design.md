...
| DAPO clip / LR / group | **0.2, 0.2 (SYMMETRIC) / 1e-6 / 8 — CONFIRMED by activity 011 across 1,200 steps** (screen 66.75→81.00 monotone, entropy regulated throughout, no thermostat ever needed; λ_TEA stayed 0) | v1 §7.3 |
| | ⚠ Activity 011 pins, all measured: **contradiction penalty log-only** (69–74% FP on correct work; tail-anchored redesign owed); **LOOP_TEMPLATE_MIN_RUN 30**; strict grader + 7 deterministic equivalence classes (`normalize_ext`); **cap 16,384 for AIME-bearing pools** (at 12,288, 20–25% of hard-source cap-hits held formed answers); 8 problems/step + prefetch; per-source answer-band targets owed (band inert off-gsm8k, in-band 0.0–6.0%); B floor 120 group-relative. Endpoints: g1200 max-accuracy / g400 max-efficiency (think-per-correct 565). | |
