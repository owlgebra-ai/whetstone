...
| 011 | [011-stagec-entropy-fix.md](011-stagec-entropy-fix.md) | P7b: Stage-C rerun — symmetric clipping fixes the 010 entropy failure; **F4 substantive PASS**; 1,200-step campaign (screen 66.75→**81.00**, pooled bench 41.1→**53.4** vs original 58.4 at 2.4–3× shorter thinks); pool widened to 9,350 (MATH/AMC/AIME, double-gated); cap 12,288→16,384 at g900; two audit-driven reward repairs + grader equivalence ext; endpoints g1200 (accuracy) / g400 (efficiency); P8 unblocked | done |
| 010 | [010-stagec-rl.md](010-stagec-rl.md) | P7: Stage C segment-routed DAPO + **F4 gate**. Part 0/1 built and unit-tested (131 tests); 4,000-problem bucket table (**79.6% mixed groups**); 60-step pilot. **F4 FAILS both clauses** — no checkpoint Pareto-dominates the init (think-per-correct 331→367) and `missing_think_close` worsens 5.6%→35.6% at the training sampler. Diagnosis: Stage C's think side is entropy-raising end to end with no ceiling, applied to a checkpoint F3c showed was already restored to 10.1× baseline | done |
| 009 | [009-stageb-assimilation.md](009-stageb-assimilation.md) | P6: Stage B assimilation SFT + **F3 gate**. Register installs and think compresses **7.2×**, entropy restored — but accuracy falls **23–25 pts**. **F3 FAIL.** ZPD band-pass cannot install the register unaided (whitelist floor is the fix; stripping and re-dressing the entry token both refuted); round 2 strictly worse than round 1; register has no backtracking primitive → degenerate loops | done |
