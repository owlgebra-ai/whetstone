...
- **~~Δlogp gate~~ — RETIRED during execution (activity 005 findings 9 and 11).** `scripts/perplexity_score.py` still runs and its `--mask-conclusion` flag is kept for the record, but it no longer gates this packet. Reason: any `P(gold | q, compact)` metric scores highest when the answer is literally in context, so it cannot separate "derived 30 and wrote 30" from "just wrote 30". Measured three ways — 17% of gate-*passing* traces judged unfaithful in exactly that shape; the more faithful GLM corpus passing *less* often than the terser Qwen3 one (58% vs 70%); and masking the conclusion failing to fix it (56% vs 63%), because the gold string survives masking in 54–61% of traces.
- **Replacement: `scripts/structural_gate.py`** — card §1.4's "never elided" column measured against each trace's own verbose source (branch retention, verification retention, value coverage), deterministic and model-free. Each check fires only when the source warrants it. Run on spark; spark rules from activity 001 still apply (`source .venv/bin/activate`, `VLLM_USE_FLASHINFER_SAMPLER=0`; the served scorer is port **8100** — 8000 is llama-swap, don't touch).
- **Target:** 300–1,000 accepted traces spanning all level bands. If the register itself is the problem (systematically failing on one problem type), that's register-card feedback for the user, not a threshold problem — report it.
- **TWO corpora now, not one** (activity 005 findings 10 and 12): `seed_register_qwen` (Qwen3-compressed → Round-0 inoculation, H_pivot, verbose control; **annotated but NOT filtered**, because those consumers need the student's own distribution to stay representative) and `seed_register_glm` (GLM-5.2-compressed, an attested central-model deviation → Stage-A teacher conditioning; filtered on `structural_pass`). Both are built from the **same 1,200 inputs**, so they are paired.

**Output:** `/data/whetstone/corpora/seed_register_{qwen,glm}/` with fields: `_uid`, `prompt`, `verbose_think`, `compact_think`, `answer`, `level`, `structural_*`, provenance (card sha, sampling params, and — on the GLM side — `compressor_model` + `central_model_deviation`).
