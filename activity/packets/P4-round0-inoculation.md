...
**Stopping:** first of S1 / S2 / S3. κ_max and x are *declared placeholders* (design §12.6) — this run's job is to measure and pin them; start with κ_max such that S2 wouldn't fire in the first 40 steps, and x = 10%. **τ_spike's ≈4-nat placeholder is DEAD ON ARRIVAL for this model (activity 004): the pre-inoculation register p95 gap is already 2.375 nats, so S1 at τ_spike=4 fires at step 0 and the run no-ops while appearing to pass.** Set τ_spike from data: strictly below the measured step-0 register p95 (~2.4) and above the verbose-control baseline (p95 d_t = 0.750) — start ~1.2, record the step-0 measurement as the reference, and pin the final value in the journal + §12.6 table. **Overshoot signature** (design §7): verbose-control likelihood falling while register p95 keeps dropping past τ_spike → roll back to the checkpoint at the S1 crossing.
