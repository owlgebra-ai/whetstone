...
| Rollout cap | **8,192 tokens** (truncates 0/377 well-formed generations; starves the loops — 009 Run 12) |
...
4. **Phase-1 bucketing:** K=8, T=0.7, 8,192 cap on all 4,000 problems under the init checkpoint (~1 h on turing at measured screen throughput). Buckets: 0/8, 1–7/8, 8/8, **split by seen/unseen** and by level. This table is Phase 1's curriculum AND the memorization check's baseline — if seen problems bucket dramatically easier than unseen at the same level, say so loudly before training on them.
